...
%%
%% Object re-verified 2026-08-06 by two standard-library verifiers
%% (verify_cfr525.py, shipped with this note; verify_cfr.py, in the
%% program's construct/florentine directory). They are NOT code-disjoint:
%% 26 of the shipped checker's 104 executable lines appear in the second
%% (see section sec:verif). Both confirm all 5*25*24 = 3000
...
(\texttt{cfr\_search.c}) in the normal form $\phi_0=\mathrm{id}$,
$\phi_i(0)=0$: the identity first row is fixed by that normal form, and the
remaining four rows are a clique in the compatibility graph on the
$K$-equivariant orthomorphisms of $\Zn_{25}$ for the prescribed multiplier
group $K=\langle 7\rangle=\{1,7,18,24\}\le\Zn_{25}^{\ast}$; none of that
...
second verifier that is not code-disjoint from it (\S\ref{sec:verif}). The object establishes
...
\cite[62.22(1)]{HCD}; the remaining three rows are non-linear (this is
confirmed by the second verifier \texttt{verify\_cfr.py} of
\S\ref{sec:verif}, which finds no multiplier $c$ with $R[t]\equiv ct$ for
rows $2$, $3$, $4$; the shipped \texttt{verify\_cfr525.py} performs no
linearity test). The recorded
...
\subsection*{A second verifier, and what it shares}
...
first and from the search. It re-checks the same object through separate code
...
(\texttt{OVERALL: OBJECT IS A VALID CFR(5,25): True}). The two verifiers are
not code-disjoint: $26$ of the shipped checker's $104$ executable lines also
appear in the second, comprising the argument and file-loading scaffolding, the
loop headers, the two-line \texttt{is\_permutation} predicate and the
circular-distance expression, each of which admits one obvious formulation; the
longest identical run is five lines, the header of the distinctness double loop,
and the longest purely input/output run is three. The equivariance
test, the linearity classification and the second event-counting method are its
own. The agreement is therefore agreement between two different check batteries
over shared plumbing, not between two independent programs.
...
independent ways and reproduced by a second verifier that shares $26$ of the
shipped checker's $104$ executable lines (\S\ref{sec:verif}).
