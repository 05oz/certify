...
Hessian spectrum is $\{-11.9910\ldots,\,+1.6899\ldots\times10^{-4},\,
+1.1300\ldots\}$~MHz/mT$^2$, each bracket of width below $2.1\times10^{-9}$.  The
signatures are mixed: none of the ten published zero-field transitions has a
definite Hessian, so at zero field each of these frequencies is a saddle in
$B$.
...
lower bound of
more than $\mathbf{7.9\times10^{5}}$ per site ($1.6\times10^{6}$ for both sites)
...
the other eighteen entries agree with our brackets to $\le0.06$~MHz.  The two
values appear to be stale.
...
battery (corrupted inertia count, shifted Hessian bracket, narrowed gradient
...
\begin{question}
The physical atlas.  The certified signatures (all published points saddles or
maxima, none minima) and the certified zero-field curvatures are properties of
the model \eqref{eq:H} with the printed matrices.  Do measured second-order
