...
in full (arXiv v2; a copy is archived in the author's working tree, not
redistributed here, and all
...
$(f\mkern-2mu2)$; the captured control log's closing total for order $16$
prints $15692$, which is the sum of all three failure lines in that block
including the single base-claim failure \texttt{BASEFAIL 1}. The
strong-form total, excluding the base failure, is $15{,}691$, as the
\texttt{RSUMMARY} on the next line records with
\texttt{sfail=15691}). Each checker was likewise run on deliberately doctored
...
$3$-connected. Each corruption is rejected by the gate that targets it,
with a nonzero exit code, while the intact certificate in the same file
is accepted. The membership gate is \texttt{refcert.py}'s alone: run with
\texttt{-{}-g6set} and \texttt{-{}-check3c} it rejects all eight, whereas
\texttt{verify\_cert.py}, which tests no membership in a generator
stream, rejects seven and accepts the relabelling with exit code $0$ (it
exits on the first violation, so the control is run one line at a time).
Two controls on the controls confirm the diagnosis, since
...
\emph{Certify} (\url{https://github.com/05oz/certify}; concept DOI
\href{https://doi.org/10.5281/zenodo.21799111}{10.5281/zenodo.21799111}),
as Part~I, version DOI
\href{https://doi.org/10.5281/zenodo.21897011}{10.5281/zenodo.21897011}.
...
behind, and the mitigation is count agreement with the independent
published enumerations --- \cite{BGM11} at every order and \cite{McKR86}
at orders $10$ through $20$, its tables stopping there ---
