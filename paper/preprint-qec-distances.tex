...
standard-library readers, $1{,}128$ lines in total, with no solver and no proof
...
and a trusted base of four standard-library Python readers, $1{,}128$ lines,
...
\subsection{Software}
Four files, $1{,}128$ lines of Python in total:
\texttt{check\_witness.py} ($95$), \texttt{check\_duality.py} ($73$),
\texttt{check\_lower.py} ($481$), and \texttt{check\_prof.py} ($479$). The
first three, $649$ lines, are the checkers the independent audit of
\S\ref{sec:audit} read and exercised; the fourth was added with the August~6
extension to verify the profile-normalisation certificates behind $d=18$ at
$n=288$ and the $n=360$ lower bound, and shares no code with the other three.
...
was re-run against the shipped $481$-line checker before release --- $20$
