...
The check works from the raw map `F` (note §1.2, the displayed map, eq. (2)) in exact arithmetic throughout (SymPy over ℚ). It was not written from scratch, and the four scripts below are not code-independent of one another or of the rest of `scripts/`. All four transcribe eq. (2) in the same three lines, byte for byte, and those three lines are also `scripts/dixmier_symplectic_verify.py:15-17`, which the repository has carried since v0.1.0. The `z`-elimination giving `h₁, h₂` is one routine shared by three of them: `fibre_check.py:make_h` and `exhibit.py:h1h2` have identical abstract syntax after renaming, `fibre_check.py:100-102` is byte-identical to `exhibit.py:18-20` and to `exhibit2.py:14-16`, and `structural.py:80-81` carries the same two formulas spelled differently. The downstairs `A, B, C` and the literals `Δ₁, Δ₂` are shared as well: `fibre_check.py:155-158` and `:164-165` are byte-identical to `structural.py:27-30` and `:75-76`. And `exhibit.py` is, as the table below says, a variant of `exhibit2.py` — 17 of its 66 executable lines are verbatim there, in byte-identical runs of 7 and 4.

Read the four scripts, therefore, as one check performed four ways over a single transcription, not as four independent confirmations. The transcription is the common-mode risk this leaves: if eq. (2) were mistranscribed, the gate and all three exhibits would carry the same wrong map and would agree on a wrong fibre count, and `exhibit.py` and `exhibit2.py` contain no assertions that could catch it. What constrains the transcription sits inside `fibre_check.py` alone — `det JF = −2` and the three `coeff_z(F_i)` identities, asserted at lines 90 and 95-97, the latter also fixing the two multipliers the elimination uses.

What the scripts do not share is the *upper* bracket, and that is where the gate earns its name. `fibre_check.py` brackets each fibre from both sides, and the two agree wherever both return a number. The lower side is not unshared: `fibre_check.py:140-141` counts distinct roots as `deg P − deg gcd(P, P′)`, which is `exhibit.py:35-36`, `exhibit2.py:38-39` and `structural.py:88-89` with `x` in place of the generic `w` — and `sqf = px.gcd(px.diff(x))` is byte-identical in those three. `structural.py`'s 113-point sweep is counted by that same line. What nothing else under `scripts/` does is `length_with_mult`: the Gröbner staircase, standard-monomial count. Add the seven separating forms `w = ax + by`, the `Δ₁ = 0` rational-point scan, `fibre_check.py`'s four assertions, and `structural.py`'s three (`det J_G = 2C²`, the contraction of `{C = 0}` to `(1,0)`, and the vanishing `u²` coefficient), which have no counterpart in the other three. Read the plurality that way: one transcription, one distinct-root count, and exactly one independent second opinion on each fibre.

The scripts ship in the repository under `scripts/erratum-check/`:
...
Each terminates without error. (SymPy over ℚ; no OS-specific calls.) `exhibit.py` prints the same radical preimages as `exhibit2.py` but uses `radsimp` and runs slowly; it is included for completeness only. Two qualifications, added 2026-08-14: "all assertions passing" was vacuous for `exhibit.py` and `exhibit2.py`, which contain no assertions; and the agreement of `exhibit.py`'s preimages with `exhibit2.py`'s was asserted here without ever having been checked, and was in fact false until the Γ-point fix of 2026-08-13 (see `GATE-LOG-ENGINE2-2026-08-12.md` R6).
