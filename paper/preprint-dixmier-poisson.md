...
| `ms_*_c{0,32003}.ms`, `out_*.txt` | the eight leaf systems, char 0 and mod 32003 (`out_*_q.txt` = char 0, `out_*_p.txt` = mod 32003); all sixteen outputs are the reduced Gröbner basis [1]. The scripts that generated these inputs are not part of the deposit |
...
Expected: every `out_*_q.txt` and `out_*_p.txt` contains the reduced Gröbner basis [1], matching the stored outputs. The msolve runs above are not expected to time out; `min_verify.py` part `II` is expected to print `TIMEOUT` on both leaves; a timeout is inconclusive, not a failure, and the Branch-II proof is the stored msolve output.
