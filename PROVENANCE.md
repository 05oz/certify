...
|---|-------|--------|------------------------|--------|
| 16 | A certificate format for quantum minimum distance in which **neither the solver nor the shipped CNF is in the trusted base**: the checker regenerates the CNF from the raw parity-check matrices and machine-checks the three algebraic side conditions that make the encoding exact | **ours** | `qec-scripts/check_lower.py` (488 lines), Theorem 3.1 of the paper | `python3 qec-scripts/check_lower.py qec-certificates/<code>/lower_*.json` |
...
3. The shipped `check_lower.py` is 488 lines; the auditor read a 419-line version. The difference is an optional totalizer cardinality encoding that **no certificate in this release selects**. See Remark 4.1 of the paper.
