...
%%     at n = 9, 10.
%% Kabiya--Yuster read in full 2026-08-05 from the copy saved with the
%% artifacts (primary-source-kabiya-yuster-tt3.pdf; author preprint).
%% Yuster 2004 checked verbatim against arXiv math/0304180 the same day
%% (copy archived with this note). Fix pass 2026-08-05: see FIXLOG.md.
%% ==========================================================================
...
read in full on August~5, 2026, from the copy saved with the artifacts (an
...
The artifact directory (\texttt{problem-2/} in this program's research
tree, distributed with this note) contains the complete pipeline,
sweeps, certificates, and verifiers: $148$\,MB of gzipped $n=10$ sweep
logs in sixteen files, $2.3$\,MB for $n=9$, and everything else under
$2$\,MB. The certified release is exactly the $32$ files pinned by MD5
digest in \texttt{MD5SUMS.txt} and reproduced in Table~\ref{tab:md5}:
the sweeps, certificates, minimizers, verifiers, and code. The auxiliary
files cited in the text --- \texttt{n10\_chunk\_counts.txt},
\texttt{smalln\_table.txt}, \texttt{cand9.out}, \texttt{cand10.out}, and
the archived primary source --- ship alongside them, unpinned; the
certified chain of Theorem~\ref{thm:main} runs entirely through the
pinned files.
...
DOI 10.1007/s00026-008-0352-3. Read in full on August 5, 2026, from
the copy archived with the artifacts
(\texttt{primary-source-kabiya-yuster-tt3.pdf}); that copy is the author
