...
\bottomrule
\end{tabular}
\caption{The thirteen free graphs on $20$ vertices. Even this coarse
fingerprint separates all thirteen: no two rows agree in all three
columns (e.g.\ $w_1$ and $w_5$ share the first and second, but their full
relation matrices differ; see the certification). Arc counts range over
...
cross-checked by the invariant fingerprint of Table~\ref{tab:inv}, which
already separates all thirteen; no two are isomorphic.
